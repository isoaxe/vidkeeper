...
program.version("1.0.0").action(async () => {
  try {
    ...
    const downloadType = await question(
      "What would you like to download? (v) Video or (a) Audio [default: v]: ",
    );
    ...
    if (isYouTube) {
      if (downloadType.toLowerCase() === "a") {
        ...
        console.log(
          "Merging video and audio with ffmpeg for QuickTime compatibility...",
        );
